...
export default function AppointmentForm({ propertyId }) {
    ...
    const {
        ...
        defaultValues: {
            property_id: propertyId || "69ff8d5a829dd292445c5ba3",//remove this later
            user_id: "",
            date: "",
            msg: "",
            time: { from: "", to: "" },
    ...
    const onSubmit = async (data) => {
        // localStorage
        const token = userData?.token || JSON.parse(localStorage.getItem("user_info") || "{}")?.token;
        ...
        const appointData = res.data.data

            if (res.status === 200 || res.status === 201) {
                toast.success("Inspection booked successfully!");
           localStorage.setItem("appointment_booked", JSON.stringify(appointData))
                console.log(res)
                router.push("/agent/appointments");
            }
        } catch (error) {
            // Defensive error handling: checking for server response vs network error
